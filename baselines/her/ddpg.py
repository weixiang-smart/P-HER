...
from baselines.her.replay_buffer import ReplayBuffer, ReplayBufferMotivation
...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        if self.prioritization == 'motivation':
            self.buffer = ReplayBufferMotivation(buffer_shapes, buffer_size, self.T, self.sample_transitions, \
                    self.prioritization, self.env_name, kwargs['goal_type'])
        else:
    ...
    def store_episode(self, episode_batch, dump_buffer, update_stats=True):
        """
        ...
        if update_stats:
            ...
            if self.prioritization == 'motivation':
                if not self.buffer.current_size==0 and not len(episode_batch['ag'])==0:
                    transitions = self.sample_transitions(episode_batch, num_normalizing_transitions, True)
            else:
