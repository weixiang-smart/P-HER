...
def train(policy, rollout_worker, evaluator, n_epochs, n_test_rollouts, n_cycles, n_batches, 
          policy_save_interval, save_policies, num_cpu, dump_buffer, **kwargs):
    ...
    for epoch in range(n_epochs):
        ...
        for cycle in range(n_cycles):
            ...
            policy.store_episode(episode, dump_buffer)
...
def launch(
    ...
    prioritization, binding, logging, version, dump_buffer, n_cycles, ratio, ratio_o, logdir, goal_type, subset_size, sigma, override_params={}, save_policies=True):
    ...
    train(
        ...
        num_cpu=num_cpu, dump_buffer=dump_buffer)

# some parameters
@click.command()
@click.option('--env_name', type=click.Choice(['FetchPickAndPlace-v1', 'FetchPush-v1', 'FetchSlide-v1', 'HandManipulateBlockRotateXYZ-v0', \
        'HandManipulateEggFull-v0', 'HandManipulatePenRotate-v0', 'HandReach-v0',"PandaPickAndPlaceJoints-v2","PandaPushJoints-v2","PandaReach-v2","PandaFlip-v2","PandaSlide-v2","PandaReachJoints-v2","PandaStackJoints-v2","PandaPushJoints-v2"]), default='FetchPickAndPlace-v0', help='the name of envs')
@click.option('--n_epochs', type=int, default=50, help='the number of training epochs to run')
@click.option('--ratio', type=float, default=0.7, help='the sigma of the rbf kernel, fetch use 0.5, and hand use 0.1')
@click.option('--ratio_o', type=float, default=0.7, help='the sigma of the rbf kernel, fetch use 0.5, and hand use 0.1')
...
@click.option('--prioritization', type=click.Choice(['none', 'motivation']), default='motivation', help='the prioritization strategy to be used.')
